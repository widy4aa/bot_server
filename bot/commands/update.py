...
def update(update: Update, context: CallbackContext):
    ...
    try:
        ...
        update.message.reply_text(ai_render("❌ Tidak dapat membaca user.csv untuk menentukan owner."))
        return

    if str(chat_id) != owner_id:
        context.bot.send_message(chat_id=chat_id, text=ai_render("You are not authorized to perform updates."))
        return

    context.bot.send_message(chat_id=chat_id, text=ai_render("🔄 Memulai proses update dari GitHub..."))
    ...
    try:
        ...
        context.bot.send_message(chat_id=chat_id, text=ai_render("📦 Local changes stashed."))

        # Pull latest changes
        out = subprocess.check_output(['git', 'pull'], cwd=repo_dir, stderr=subprocess.STDOUT)
        context.bot.send_message(chat_id=chat_id, text=ai_render(f"✅ Update successful:\n{out.decode()}"))

        # Try to apply stash
        try:
            stash_out = subprocess.check_output(['git', 'stash', 'pop'], cwd=repo_dir, stderr=subprocess.STDOUT)
            context.bot.send_message(chat_id=chat_id, text=ai_render(f"📥 Applied stash:\n{stash_out.decode()}"))
        except subprocess.CalledProcessError as e:
            # If applying stash failed, inform user and leave stash
            context.bot.send_message(chat_id=chat_id, text=ai_render(f"⚠️ Gagal menerapkan stash: {e.output.decode()}"))

    except subprocess.CalledProcessError as e:
        context.bot.send_message(chat_id=chat_id, text=ai_render(f"❌ Git operation failed: {e.output.decode()}"))
        ...
        context.bot.send_message(chat_id=chat_id, text=ai_render(f"❌ Error saat update: {e}"))
        return

    # Restart the bot process
    context.bot.send_message(chat_id=chat_id, text=ai_render("🔁 Restarting bot to apply updates..."))
